...
from findfunctions import extract_functions
from code_analyzer import fix_source_code
from file_parser import get_function_node
...
from docstring_inserter import insert_or_update_docstring
        ...
        # ---------------- FUNCTION ANALYSIS ----------------
            ...
            if df.empty:
                ...
                if file_path:
                    ...
                    # This generates specifically using the 'current_style' variable which 
                    # is now updated instantly when the radio button is clicked.
                    ...
                    with btn2:
                        if st.button("🚀 Update All Functions", use_container_width=True):
                            ...
                            st.balloons()
                            st.rerun()
                st.markdown("### 📄 Docstring Comparison (Before vs After)")
                ...
                if file_path:
                    ...
                    current_doc = ast.get_docstring(node) if node else None
                    doc_key = f"{selected_file}_{selected_function}_before"

                    if doc_key not in st.session_state:
                        st.session_state[doc_key] = current_doc
                    ...
                    with before_after_col1:
                        st.markdown("#### 🔴 Before (Current Code)")

                        # before_text = current_doc if current_doc else "No docstring found."
                        before_text = st.session_state.get(doc_key)

                        if not before_text:
                            before_text = "No docstring found."

                        st.markdown(
                            f"""
                            <div class="doc-card before-box">
                \"\"\" 
                {before_text}
                \"\"\"
                            </div>
